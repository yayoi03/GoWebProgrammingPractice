...
func averageColor(img image.Image) [3]float64 {
	...
	for y := bounds.Min.Y; y < bounds.Max.Y; y++ {
		for x := bounds.Min.X; x < bounds.Max.X; x++ {
			...
			r, g, b = r+float64(r1), g+float64(g2), b+float64(b2)
		}
	}
...
func cut(original image.Image,db *DB,tileSize,x1,y1,x2,y2 int)  {
	<-chan image.Image {
	...
	<-chan string {
		...
		go func ()  {
			...
			for {
				...
				case s4,ok4=<-c4:
					go copy(img,s4.Bounds(),s4,image.Point{r.Max.X/2,r.Miax.Y/2})
				}
				if (ok1&&ok2&&ok3&&ok4){
					break
				}
			...
			c <- base64.StdEncording.EncodeToString(buf2.Bytes())
		}()
		return c
	}
}
